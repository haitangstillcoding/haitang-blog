---
cover: /blog/database/mysql/images/隔离级别和现象.png
recommend: false
tag:
 - MySQL
 - 幻读
...
---
# MySQL幻读被彻底解决了吗？
之前我也提到过，MySQL InnoDB 引擎的默认隔离级别虽然是「可重复读」，但是它很大程度上避免幻读现象（并不是完全解决了），解决的方案有两种：
...
![并发事务导致的现象](./images/并发事务导致的现象.png)
...
![不同隔离级别可能发生的现象](./images/不同隔离级别可能发生的现象.png)
...
可重复读隔离级别是由 MVCC（多版本并发控制）实现的，实现的方式是启动事务后，在执行第一个查询语句后，会创建一个 Read View，**后续的查询语句利用这个 Read View，通过这个  Read View 就可以在 undo log 版本链找到事务开始时的数据，所以事务过程中每次查询的数据都是一样的**，即使中途有其他事务插入了新纪录，是查询不出来这条数据的，所以就很好了避免幻读问题。

做个实验，数据库表 t_stu 如下，其中 id 为主键。

![数据表t_stu](./images/数据表t_stu.png)

然后在可重复读隔离级别下，有两个事务的执行顺序如下：

![事务执行顺序](./images/事务执行顺序.png)
...
![select_for_update不加锁](./images/select_for_update不加锁.png)
...
![间隙锁阻塞其他事务插入](./images/间隙锁阻塞其他事务插入.png)
